...
namespace {
    ...
    bool hasExactlyOneDelimiter(const std::string& line, char delim)
    {
        size_t first = line.find(delim);
        if (first == std::string::npos)
            return false;
        return (first == line.rfind(delim));
    }
}
...
std::pair<std::string, double> parseCsvRateLine(const std::string& line, int pos)
{
    ...
    if (line.empty() || isWhitespaceOnly(line))
        throw std::logic_error(ErrorMessages::EMPTY_LINE);
    if (!hasExactlyOneDelimiter(line, ','))
