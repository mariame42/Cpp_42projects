...
/*
    the container for this program is a map
    why a map ? because we need to be able to get the exchange rate by date quickly 
    -> map save data in a sorted way.
    -> will have pairs of data -> (date, exchange rate)
*/

/*
    BitcoinExchange class
    This class is used to exchange the Bitcoin price by date

    program steps :
        1. read the exchange rate file and store it in a map
        2. read the input file and process each line
        3. for each line, get the exchange rate by date
        4. print the result by multiplying the exchange rate by the input value
*/
