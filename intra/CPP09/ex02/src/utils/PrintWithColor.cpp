...
void PrintWithColor::printRedError(const std::string& str) {
...
void PrintWithColor::printDeque(const std::deque<int>& deq) {
    std::cout << BLUE << "Values: " << RESET;
    for (std::size_t i = 0; i < deq.size(); i++)
    {
        std::cout << CYAN << deq[i] << RESET;
        if (i + 1 < deq.size())
            std::cout << " ";
    }
    std::cout << std::endl;
...
/*
    print the standard output from the subject :
        example :
            Before: 3 5 9 7 4
            After: 3 4 5 7 9
